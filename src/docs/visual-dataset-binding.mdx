...
- AppSource certified visuals are not permitted to bring in data or resources from external locations.
  ...
  - You can attempt this in the [standalone version](/installation#standalone-version) of the visual, but loading data from remote endpoints is subject to CORS restrictions due to security restrictions in all Power BI custom visuals. <OutboundLink href="https://www.html-content.com/reference/limitations#custom-visuals-high-level" target="_blank">You can read more about this here</OutboundLink>.
