...
We saw [in the worked example](simple-worked-example#adding-tooltips), that it's fairly straightforward to add tooltips to a specification, but we'll unpack on this page ina bit more detail.
...
3. Tooltip handler is fired when a mark is hovered over.
...
-   Adding a tooltip oes not immediately add visual feedback. If you want to track the position of the resolved data point more visually (e.g. like for a line chart), you will need to add a suitable set of marks to do this. Both <OutboundLink href="https://vega.github.io/vega/examples/" target="_blank">Vega</OutboundLink> and <OutboundLink href="https://vega.github.io/vega-lite/examples/" target="_blank">Vega-Lite</OutboundLink> have examples you can refer to for this.

-   Development so far has been focused on Power BI integration. It's possible that if you deviate from the above patterns, then tooltips may not display correctly. If you find any such use cases, please <OutboundLink href="https://github.com/deneb-viz/deneb-powerbi/issues" target="_blank">create an issue</OutboundLink> with the appropriate reproduction steps (e.g. example specification and/or data).
