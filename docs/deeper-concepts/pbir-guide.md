...
This guide is valid for Deneb 1.9 and above. This _should_ work with earlier versions, but you may need to manually step through or dismiss the _Create or import new specification_ dialog in the Advanced Editor for new visuals. This experience has been improved from 1.9 onwards.
:::
...
In addition to PBIR, the details on this page may also help understand Deneb's internal structure better, for use with building [report themes](https://learn.microsoft.com/en-us/power-bi/create-reports/desktop-report-themes?WT.mc_id=DP-MVP-5003712) containing Deneb visuals.
...
All Power BI visuals have a unique identifier (GUID) that identifies the visual type. For the AppSource (certified) version of Deneb, this GUID is:
...
These editions are not tied to AppSource and will require additional setup, as the .pbiviz code must be included in the PBIP assets for them to work correctly.

Much like the legacy .pbix format for Power BI workbooks, .pbiviz files are compressed folders that can be opened with a tool like 7-Zip to extract the contents for inspection. Extracting the .pbiviz file into `/CustomVisuals/{GUID}/` provides the necessary structure to include the visual in PBIR/PBIP.
...
When you add a visual to a report and start configuring it, information persisted in the JSON for that visual instance will include items that tie to this capabilities definition, allowing the visual to be regenerated from persisted state whenever you reopen the page (or report).

It can be easy to assume that these work in a particular way, but it is important to realize that (particularly in the case of persisted properties), visuals often have internal logic that acts on these values as part of their operation. So, it can therefore be assumed how they work, but this document serves to provide you with details of what properties are set and how they affect your generated Deneb visuals so that you can understand how to work with them rather than guess at what they do (and possibly do the wrong things in some cases).

## Dissecting the Simplest Scenario: a Manually Added Deneb Visual

To get an understanding of the structure, let's look at what happens when we add a Deneb visual to a report canvas, without adding data or configuring anything. The base definition may look like this:
...
- `visual.query.queryState.dataset` represents any columns or measures added to the visual's **Values** data role. This is known internally in the visual capabilities as `dataset`, which matches how Deneb maps and assigns this data in your specification.
...
Because Power BI's property system works this way, the JSON for the specification and configuration will be serialized and escaped ("[stringified](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify)") before being stored in the visual's properties. As our visual has an empty config (`{}`), the property storage should be prepared as follows:
...
Our specification and configuration are now stored under `visual.objects.vega` in the properties `jsonSpec` and `jsonConfig` respectively. These will be deserialized and parsed by Deneb when the visual is loaded in Power BI, and the visual will render as expected. So as long as your JSON string is valid (and it can contain comments, as Deneb will parse it using JSONC), Deneb will be able to work with it.
...
If we open our workbook with this configuration, we will see our simple bar chart rendered as expected:
...
The following sections provide a reference for each of the `objects` that Deneb uses to persist configuration and state. Where possible, we'll provide as much detail as we can about what each property does and how it affects the visual's operation.
...
Properties in this group are concerned with the specification and configuration for the Vega or Vega-Lite runtime within a Deneb visual instance.
...
| `logLevel`                  | `3D` (vega.Info)           | [integer](#integer)  | The [logging verbosity level](https://github.com/vega/vega-util?tab=readme-ov-file#logging) for the Vega/Vega-Lite runtime. One of: `0` (None), `1` (Error), `2` (Warn), `3` (Info).                                                                                                                                                                                                     |
...
| `enableContextMenu`         | `true`                     | [boolean](#boolean)  | Controls whether the [Power BI context menu](interactivity-context-menu) appears on right-click. Analogous to checking the _Show context menu on right-click_ checkbox in the interactivity settings.                                                                                                                                                                                    |
| `enableContextMenuSelector` | `true`                     | [boolean](#boolean)  | When the context menu is enabled, controls whether Deneb attempts to resolve the clicked data point for drill-through and other data-specific menu options. Analogous to checking the _Attempt to resolve data point-specific actions_ checkbox in the interactivity settings. Only relevant when `enableContextMenu` is `true`.                                                         |
...
| `selectionMode`             | `simple`                   | [text](#text) (enum) | The selection mode to use when `enableSelection` is `true`. `simple` will monitor the Vega view, attempt to resolve data points and apply cross-filtering. `advanced` relies upon [advanced cross filtering](interactivity-selection-advanced), which is only supported and functional when `provider` is `'vega'` and you have defined the relevant events in your spec to initiate it. |
...
| `scrollbarRadius`     | `0D`                       | [integer](#integer) | The border radius of the [container scrollbar](scrolling-overflow#configuring-scrollbar-appearance) in px when it is shown. Valid values are `0D` to `8D`.                                                                                          |
| `scrollbarWidth`      | `10D`                      | [integer](#integer) | The width of the [container scrollbar](scrolling-overflow#configuring-scrollbar-appearance) in px when it is shown. Valid values are `8D` upwards; values below `8D` are not usable for interactive handles.                                        |
...
Properties in this group are displayed in the _Data limit options_ menu and determine how Deneb handles [data row limits](dataset#data-row-limits) imposed by Power BI.
...
Properties in this group are displayed in the _Advanced Editor_ property menu in the formatting pane and concern the editor's operation and appearance. Setting them can be used to 'prime' the editor experience for users, but will have no effect on the visual rendering or operation outside of the editor.
...
The PBIR documentation does not elaborate on property specifics very well, so here are some notes on the types that may help you understand how to work with them in Deneb. Note that we only cover how we support them, not the entire PBIR property system.
...
Colors can be supplied as `Literal` values that support any known HTML/CSS color format, but the syntax is slightly different from that of regular text properties. It is also worth noting that the color value should be enclosed in single quotes because it is stored as a [text](#text) literal within the JSON structure. For example, to set a color property to red, you would use:
...
And you can use `ThemeDataValue` for binding to your theme:
