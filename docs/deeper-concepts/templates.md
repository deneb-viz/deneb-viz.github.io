...
```json title="Example datasets structure"
{
  "datasets": {
    "dataset": [
      { "key": "__dataset.0__", ... },
...
| `supportFieldConfiguration` | No       | object                                                 | Per-field [supporting field](dataset#supporting-fields) flags to persist with the template. Sparsity is at the **field** level - an entry is only included for fields whose configuration differs from Deneb's role-based defaults; fields without an entry inherit defaults on import. When present, the object must include explicit `true` / `false` values for `highlight`, `highlightStatus`, `highlightComparator`, `format`, and `formatted`; `names` and `treatAsParameter` are optional and only relevant for parameter fields. The structure mirrors the [PBIR `supportFieldConfiguration`](pbir-guide#supportfieldconfiguration), minus the outer text-literal wrapping that PBIR requires. |
...
- **Current (v2):** https://deneb-viz.github.io/schema/deneb-template-usermeta-v2.json
- **Legacy (v1):** https://deneb-viz.github.io/schema/deneb-template-usermeta-v1.json - retained for validating pre-2.0 templates before they are imported and migrated.
