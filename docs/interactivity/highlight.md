...
It is possible to make your visual respond to cross-highlight interactions from other visuals. Much like [Cross-Filtering](interactivity-selection), this requires some additional work to implement the necessary visual effects that you want your visual to have, so as such, the properties are **disabled** by default. This page shows how to enable this and provides guidance on working with the data sent to the visual dataset.

:::info This is A Deep and Info-Dense Topic
This whole page provides valuable insight into how cross-highlighting works in Power BI and the theory you need to approach it in your own visuals.

If you're the sort of person who just wants to get on with it, the one thing you absolutely need to bear in mind is: **any visual you want to build that supports cross-highlighting is really 2 visuals in one**:

1. A visual that shows the original values.
2. A visual that shows the highlighted values in context with the originals.

It is recommended that you read the page in order to better understand what cross-highlighting involves. If you want to jump straight in, you can head down to the [Recommended Approaches](#recommended-approaches) section, but for those who read the whole thing, it may make your life a bit easier 🙂
:::

## Filtering vs. Highlighting: Conceptually

Power BI offers three types of [interactions between visuals](https://learn.microsoft.com/en-us/power-bi/create-reports/service-reports-visual-interactions?WT.mc_id=DP-MVP-5003712&tabs=powerbi-desktop): **Filter**, **Highlight**, and **None**.

As creators, much of what goes on under the hood is abstracted from us through visuals we directly consume in Power BI. If we want to tear off the mask and manage this ourselves, we need to understand it at a basic level. What follows is a simple guide on how these interactions may work (this can also be dug into at a lower level [in the custom visuals SDK documentation](https://learn.microsoft.com/en-us/power-bi/developer/visuals/highlight?WT.mc_id=DP-MVP-5003712&tabs=Standard)).

Let's say we have a chart which shows **Sales** by **Country Code**, but we intend to build a visual with Deneb that uses **Sales** by **Product**. For now, we will represent this using a table visual:
...
It should be fairly evident that the totals in our table have been updated to reflect the values from the _US_ category. In our Deneb dataset, the **Sales** field will contain the same value, and our chart will update as we expect.

### Highlighting

With the **Highlight** interaction, we want to show the values in context with the original values. With this enabled, things change a bit. Our dataset now looks similar to the following representation:
...
With the above in mind, before we talk about the strategy and how to work with cross-highlighting in Deneb, there are some other considerations.
...
Whilst this is not intuitive, it is technically correct. However, we understand that there's a lot to think about when learning a language, and that you need to understand more about how Power BI works under the hood for this kind of thing.
...
Therefore, we want to try to meet you halfway. As such, we assume that cross-highlighting functionality should not be on by default. If you want to opt in to add cross-highlight functionality in your visual, it's a 2-stage process. This is detailed further below.

## Recommended Approaches

You probably want to get into this as quickly as possible, so use the appropriate directions under each section to set up the visual as needed for the situation you want. However, the sections that follow afterward are pretty much recommended reading if you want to understand how or why things work the way they do, and what you need to understand what's happening inside your visual.
...
- Ensure that the **Expose cross-highlight values for measures** property is disabled in the _Cross-highlighting_ section of the [**Project setup** pane](visual-editor#settings-pane) of the Visual Editor. This should be disabled by default, unless you have loaded a template that utilizes it.
...
Note that this may not be 100% reliable, but it is the best we can do due to custom visual limitations. If leaving this as-is, you should really ensure that your data is correct for your readers, and explicitly set the correct interactions if not.
:::
...
- Ensure that the **Expose cross-highlight values for measures** property is enabled in the _Cross-highlighting_ section of the [**Project setup** pane](visual-editor#settings-pane) of the Visual Editor. This should be disabled by default, unless you have loaded a template that utilizes it.

- Confirm that any other visuals that can interact with your visual are set to **Highlight** your visual in the visual header.

To learn more about how this works at a lower level and how to access the necessary fields in your dataset to leverage the functionality, keep reading.

## Cross-Highlighting Strategy

Now, we can talk about how to get started. As this is a dense topic, some points may be repeated from above to ensure everything is laid out in one place.

- You can configure whether Deneb should expose highlight values to the dataset through the **Expose cross-highlight values for measures** property, which is enabled in the _Cross-highlighting_ section of the [**Project setup** pane](visual-editor#settings-pane) of the Visual Editor.
...
- If there is no active selection in another visual, the highlight and original values of measures are equal to each other (but still exist). You can get a visual representation of what this might look like in our [walkthrough of the concept above](#highlighting).
...
Where [cross-filtering](interactivity-selection) works for the whole row, cross-highlighting works at the measure-level. If the **Expose cross-highlight values for measures** property is enabled, then your visual dataset contains additional fields for each measure.

- `[measure name]__highlight` - this is the highlight value for the measure, as opposed to its original value, which is stored in the regular measure field. This provides a way to encode original vs. highlight.

- `[measure_name]__highlightStatus` - this provides additional state about this specific measure for this particular row context, indicating whether a highlight has been applied or not (like the `__selected__` field, but more specific). This can be one of the following values:
  ...
  - **`neutral`**: there is no cross-highlight state applied to the visual. The original and highlight values will be equal.

- `[measure_name]__highlightComparator` - this provides a pre-calculated way of determining if a highlight value is equal to its original value or not, and allows you to apply conditional encodings without the need to derive this separately in a `calculate` transform or similar. This can be one of the following values:
...
![cross-highlight-dataset-01.png](./img/cross-highlight-dataset-01.png "A bar chart, showing Sales by Country, and an accompanying table visual showing Product and corresponding Sales amount. The table also shows Deneb's 'Highlights' version of the Sales field, which is the same as the original because no active cross-highlight is applied. There is also a 'Highlight Status' field, which shows 'neutral', and a 'Highlight Comparator' field, which shows 'eq', again because there is no active cross-highlight applied.")
...
![cross-highlight-dataset-02.png](./img/cross-highlight-dataset-02.png "A bar chart, showing Sales by Country, and an accompanying table visual showing Product and corresponding Sales amount. The table also shows Deneb's 'Highlights' version of the Sales field, and the 'Highlight Status' and 'Highlight Comparator' fields.  The bar chart's US value is highlighted, and the table shows the original Sales amount for all Products (irrespective of Country), as well as the current highlight value for the US. The 'Highlight Status' field shows 'on', indicating the measure is actively highlighted. The 'Highlight Comparator' value shows 'lt', which specifies that the highlighted value is less than the original value.")

## Managing Highlight State Visually Through Encodings

As with cross-filtering, you will need to ensure that you design your visual and its encodings to effectively communicate cross-highlight status to your readers. This is often uncharted territory when it comes to bespoke visuals in Power BI, as there are often no existing visuals or patterns to crib ideas from.

It's generally regarded as trying to communicate where your highlights are vs. your original values, so that your readers can contextually resolve this information.

The [Simple Worked Examples](#simple-worked-examples) section below shows how you may be able to get started with exploring this further. While these focus on Vega-Lite, both versions of the _Simple Bar Chart_ template available in the _New Specification_ dialog include simple cross-highlighting bindings to help you get started.

## Limitations and Considerations

:::info This Sounds Hard...
Well, it's not always straightforward! It is early days for this type of "visual thinking" in Power BI, and the community will likely uncover better design patterns than those outlined on this page. They will certainly help you get on your way.

If you have found an alternative approach or a great idea for how to help us improve this, please [let us know](https://github.com/deneb-viz/deneb/issues)! We would love to continue evolving this functionality and make it more accessible if we can.
:::
