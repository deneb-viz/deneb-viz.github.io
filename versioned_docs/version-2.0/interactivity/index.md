...
Power BI provides the ability to create visuals using [R](https://learn.microsoft.com/en-us/power-bi/create-reports/desktop-r-visuals?WT.mc_id=DP-MVP-5003712) and [Python](https://learn.microsoft.com/en-us/power-bi/connect-data/desktop-python-visuals?WT.mc_id=DP-MVP-5003712). Whilst these extend your options significantly, these are typically rendered as a static image and mean that it's currently not possible for you to interact back from these visuals to the rest of the report.

On occasion, these visuals may also require additional dependencies if you want to share with a wider pool of end users, or a library may not be available in the Power BI Service during deployment.

## How Deneb Can Be Different from R & Python Options

In addition to the JSON-based declarative syntax, Deneb includes the Vega and Vega-Lite libraries. Because they are built for the web, anything you produce should ideally be portable within the Power BI ecosystem, irrespective of which device you're using.

Both [Vega](https://vega.github.io/vega/docs/event-streams/) and [Vega-Lite](https://vega.github.io/vega-lite/docs/selection.html) have their own approaches to interactivity, and it's only logical to start thinking about how we can leverage this to make the visual "fit in" with the functionality offered by other Power BI visuals...
...
3. Prescribing how our visual should behave and respond _visually_ when our end-users interact with it.

As such, we have some considerations when delivering functionality and interactivity so that you can produce visuals that look and feel at home in Power BI, and so your end-users get a consistent experience with your visual vs. others.

The following will attempt to summarize some of the considerations we need to think about, and this is a challenge to do so concisely. For a deeper understanding, you're welcome to dive into the [developer documentation for visuals](https://learn.microsoft.com/en-us/power-bi/developer/visuals/?WT.mc_id=DP-MVP-5003712).
...
After doing this, Power BI will generate the following query against the data model (which can be verified using the [Performance Analyzer](https://learn.microsoft.com/en-us/power-bi/create-reports/performance-analyzer?WT.mc_id=DP-MVP-5003712)):
...
For these situations, Power BI provides APIs for visuals to request that Power BI handle them on their behalf. For Power BI to fulfill this, it needs to know which row(s) from the original dataset it has to resolve.
...
Any functionality we deliver needs to take these constraints into account.
...
- `__row__` - this is the (zero-based) index of the visual dataset's corresponding entry. The presence of this field indicates that the datum can be reconciled back to the original dataset; therefore, Power BI can resolve row context for interactivity events.
...
- `[measure_name]__highlightStatus` - this provides additional state about this specific measure for this particular row context, indicating whether a highlight has been applied or not (like the `__selected__` field, but more specific).
...
There are a couple of interactive scenarios we've missed out on above:

- Default tooltips are different from report page or modern tooltips.
  - In these cases, Deneb can pass a list of values to display and ask Power BI to display them inside a standard tooltip.
  - This is also the fallback behavior if the row context cannot be resolved by Power BI, or a suitable tooltip page cannot be found.
  - Tooltip functionality within Deneb is covered in much more detail on the [Tooltips](interactivity-tooltips) page.
- Slicer filtering works differently from cross-filtering, but as Deneb is not a slicer, we're skipping that one.
